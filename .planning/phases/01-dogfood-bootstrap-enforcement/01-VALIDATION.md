...
- **After every plan wave:** Run full suite command (from wave 3 on; before that, run all existing checks individually)
- **Before `/gsd-verify-work`:** `task verify` green + all CI workflows green on GitHub
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 01-01-T1 | 01 | 1 | AGENT-05 | T-01-01/02 | tiers+waivers parse, T3 self-listing | assertion | inline `node -e` shape check + `git check-attr` | ❌ created by task | ⬜ pending |
| 01-01-T2 | 01 | 1 | AGENT-06, AGENT-09 | T-01-03 | 6 approved specs, no .planning links | assertion | inline `node -e` over specs/00[1-6] | ❌ created by task | ⬜ pending |
| 01-01-T3 | 01 | 1 | AGENT-05 | — | verbatim constitution copy | assertion | `git diff --no-index` product↔docs | ❌ created by task | ⬜ pending |
...
| 01-04-T1 (RED) | 04 | 2 | AGENT-03 | T-01-10/12 | deny contract incl. crash fail-closed | unit (node:test) | `node --test .claude/hooks/tests/` (must FAIL) | ❌ Wave-0-equivalent: created RED-first | ⬜ pending |
...
| 01-05-T2 | 05 | 2 | FOUND-02 | T-01-16 | task up all-healthy on Windows | integration (local) | `task --list` + report assertion | ❌ created by task | ⬜ pending |
| 01-05-T3 | 05 | 2 | FOUND-02 | — | D-04 split green in CI | integration (CI) | `gh run list --workflow=os-matrix.yml` | ❌ created by task | ⬜ pending |
| 01-06-T1 (RED) | 06 | 2 | FOUND-12 | T-01-17/18 | rename edge cases incl. traps | unit (node:test) | `node --test scripts/tests/` (must FAIL) | ❌ created RED-first | ⬜ pending |
...
| 01-07-T1 (RED) | 07 | 2 | GATE-12 | T-01-21 | CRLF/orphan/oversize/vacuous cases | unit (node:test) | `node --test scripts/checks/tests/claude-md.test.mjs` (must FAIL) | ❌ created RED-first | ⬜ pending |
...
| 01-09-T1 (RED) | 09 | 3 | GATE-10 | T-01-25..28 | 11-case verdict matrix | unit (node:test) | `node --test scripts/checks/tests/plan-compliance.test.mjs` (must FAIL) | ❌ created RED-first | ⬜ pending |
...
| 01-09-T3 | 09 | 3 | GATE-10, AGENT-09 | T-01-29/30 | dual triggers, merge-commit only | CI + platform read-back | `gh api repos/... --jq` merge flags + run URL | ❌ created by task | ⬜ pending |
| 01-10-T1 | 10 | 3 | AGENT-01 | T-01-32 | template renders ≤200, smoke non-vacuous | check CLI | `node scripts/checks/claude-md-check.mjs --root-template ...` | ❌ created by task | ⬜ pending |
| 01-10-T2 | 10 | 3 | AGENT-01 | T-01-31 | tree files ≤150, hallucination warnings | check CLI | `node scripts/checks/claude-md-check.mjs --tree-file ...` | ❌ created by task | ⬜ pending |
| 01-10-T3 | 10 | 3 | GATE-12 | T-01-31 | gate blocking in CI | integration (CI) | `gh run list --workflow=claude-md-check.yml` | ❌ created by task | ⬜ pending |
| 01-11-T1 | 11 | 3 | AGENT-02 | T-01-33 | plan/verify skills full | assertion | inline `node -e` content check | ❌ created by task | ⬜ pending |
| 01-11-T2 | 11 | 3 | AGENT-02 | T-01-33 | skeletons + skills-lint | check CLI | `node scripts/checks/skills-lint.mjs` | ❌ created by task | ⬜ pending |
| 01-11-T3 | 11 | 3 | AGENT-04, AGENT-08 seed | T-01-34/35 | lockstep + meta lint + verify spine | unit + integration | `node --test scripts/checks/tests/lints.test.mjs` + `task verify` | ❌ created by task | ⬜ pending |
...
| macOS `task up` one-time verification | FOUND-02 (D-04) | No macOS hardware in this environment | Run `task up` on a Mac, record runtime + health results into .planning/spikes/local-stack-verification.md (PENDING row) |
| Spec approval integrity (6 Approved-by lines) | AGENT-06/09 | Human H2 act by definition | Developer confirms the six approvals reflect real review (plan 01-01 human-check) |
